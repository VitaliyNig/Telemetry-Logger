...
/// <summary>
/// Builds the <see cref="ProtocolLookups"/> bundle for packet format 2025 — the
/// original F1 25 release. Track / team / session / penalty tables are static data
/// pulled from the F1 25 v3 UDP spec appendix.
/// </summary>
internal static class Format2025Lookups
{
    ...
    // F1 25 v3 spec appendix. teamId in this format is uint8 (≤ 255) but we expose ushort
    // so the dictionary key matches ParticipantData.TeamId across all formats.
    ...
    // F1 25 only: Haas (teamId=7) slot 0 is red — indistinguishable from Ferrari. Use slot 1.
