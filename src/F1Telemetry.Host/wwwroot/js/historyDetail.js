            ...
            function renderLapList(carIdx) {
                ...
                list.querySelectorAll('.tc-lap-option').forEach(function (btn) {
                    btn.addEventListener('click', function () {
                        ...
                        state.driverSelection.set(pickedCar, { lap: pickedLap, ghost: existing ? existing.ghost : false, enabled: existing ? existing.enabled !== false : true });
        ...
        function render() {
            ...
            if (opts.compareCardMode) {
                ...
                selected.forEach(function (item) {
                    ...
                    cards += '<div class="tc-lap-card ' + (item.sel.enabled === false ? 'is-disabled' : '') + '" data-car="' + item.carIdx + '">'
                        + '<button type="button" class="tc-lap-card-remove" data-car="' + item.carIdx + '" title="Remove lap">×</button>'
                        + '<div class="tc-lap-card-top">'
                        + '<span class="driver-dot" style="background:' + teamColor + '"></span>'
                ...
                container.querySelectorAll('.tc-lap-card[data-car]').forEach(function (card) {
                    card.addEventListener('click', function (ev) {
                        if (ev.target.closest('.tc-lap-card-remove')) return;
                        var carIdx = Number(card.dataset.car);
                        var existing = state.driverSelection.get(carIdx);
                        if (!existing) return;
                        existing.enabled = existing.enabled === false ? true : false;
                        state.driverSelection.set(carIdx, existing);
                        if (opts.onChange) opts.onChange();
                    });
                });
                container.querySelectorAll('.tc-lap-card-remove').forEach(function (btn) {
                    btn.addEventListener('click', function (ev) {
                        ev.stopPropagation();
                        var carIdx = Number(btn.dataset.car);
